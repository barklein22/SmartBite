...
- OpenAI API based intent detection and natural final responses.
- Internal database first: restaurants, reviews, orders, customers and menu data.
- Google Places fallback when internal data is missing. Requires `GOOGLE_PLACES_API_KEY` in Render.
- Cosine Similarity model for similar restaurant recommendations.
- Recipe/cooking questions are treated as outside the SmartBite restaurant domain.
- Typing indicator with animated three-dot bot bubble.
