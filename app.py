...
If the user asks for recipes, cooking instructions, ingredients, or how to cook something, return intent "off_topic" because SmartBite recommends restaurants but does not provide recipes.
If the user replies only with a city or region and context.pending_ai_action exists, infer the intent from pending_ai_action.
...
Return JSON with these fields:
{
  ...
  "city": string|null,  // Use English city value like "Jerusalem" or "Tel Aviv" even if user writes Hebrew.
...
def parsed_to_params(parsed: Dict[str, Any]) -> Dict[str, Any]:
    """
    Converts the JSON returned from ChatGPT into the same params structure
    used by the local SmartBite filtering engine.
    """
    params = {
        "city": None,
        "region": None,
    ...
    # Normalize values returned by ChatGPT.
    # Sometimes the model returns Hebrew values such as "ירושלים" instead of "Jerusalem".
    # The internal CSV uses English values in columns such as city/region/cuisine.
    if params.get("city"):
        city_value = str(params["city"]).strip()
        params["city"] = CITY_MAP.get(city_value, city_value)

    if params.get("region"):
        region_value = str(params["region"]).strip()
        params["region"] = REGION_MAP.get(region_value, region_value)

    if params.get("cuisine"):
        cuisine_value = str(params["cuisine"]).strip()
        params["cuisine"] = CUISINE_MAP.get(cuisine_value, cuisine_value)

    return params


def is_recipe_request(message: str) -> bool:
    """
    SmartBite is a restaurant agent, not a recipe generator.
    Recipe/cooking instructions are intentionally treated as out-of-domain.
    """
    msg = normalize_text(message)
    recipe_words = [
        "מתכון", "מתכונים", "איך מכינים", "איך להכין", "הוראות הכנה",
        "מצרכים", "רכיבים", "recipe", "recipes", "how to make", "ingredients",
        "cook", "cooking instructions",
    ]
    return any(word in msg for word in recipe_words)
...
def search_google_places_legacy(query: str, language: str = "he", limit: int = 5) -> list[Dict[str, Any]]:
    """
    Backup external search using the legacy Google Places Text Search endpoint.
    This is used if Places API (New) returns no results or the new endpoint is not available.
    """
    ...
    try:
        ...
        print("Google Places legacy search error:", exc)
        return []
    ...
    for place in data.get("results", [])[:limit]:
        places.append({
            ...
            "source_endpoint": "Google Places Legacy Text Search",
        })

    return places


def search_google_places_text(query: str, language: str = "he", limit: int = 5) -> list[Dict[str, Any]]:
    """
    External fallback using Google Places API Text Search.
    Used only when the internal SmartBite database does not contain enough data.
    The function first tries Places API (New), then falls back to the legacy Text Search endpoint.
    """
    api_key = get_google_places_key()
    if not api_key or not query:
        return []
    ...
    try:
        ...
        print("Google Places new search error:", exc)
        return search_google_places_legacy(query, language=language, limit=limit)
    ...
    for place in data.get("places", [])[:limit]:
        ...
        places.append({
            ...
            "types": place.get("types", []),
            "source_endpoint": "Google Places API New Text Search",
        })
...
def build_google_query(message: str, parsed: Dict[str, Any], params: Optional[Dict[str, Any]] = None) -> str:
    params = params or {}
    parts = []

    restaurant_name = parsed.get("restaurant_name")
    if restaurant_name:
        parts.append(str(restaurant_name))

    cuisine = parsed.get("cuisine") or params.get("cuisine")
    if cuisine:
        parts.append(str(cuisine))

    restaurant_type = parsed.get("restaurant_type") or params.get("restaurant_type")
    if restaurant_type:
        parts.append(str(restaurant_type))

    city = parsed.get("city") or params.get("city")
    if city:
        parts.append(str(city))

    region = parsed.get("region") or params.get("region")
    if region and not city:
        parts.append(str(region))

    # Keep the query restaurant-focused even if the user's wording is vague.
    parts.append("restaurant Israel")

    if len(parts) <= 1:
        return f"{message} restaurant Israel"

    return " ".join(parts)


def external_places_payload(
    message: str,
    parsed: Dict[str, Any],
    language: str,
    params: Optional[Dict[str, Any]] = None,
    reason: str = "internal_database_missing",
) -> Dict[str, Any]:
    query = build_google_query(message, parsed, params)
    places = search_google_places_text(query, language=language, limit=5)

    # Extra retry: if the first structured query failed, try the user's wording directly.
    # This helps with famous restaurant names written in Hebrew, for example "מחניודה בירושלים".
    if not places and message:
        direct_query = f"{message} מסעדה ישראל"
        places = search_google_places_text(direct_query, language=language, limit=5)
        if places:
            query = direct_query

    return {
        "type": "external_google_places_fallback",
        "source": "Google Places API",
        "reason": reason,
        "query": query,
        "places": places,
        "fallback_answer": (
            "לא מצאתי מספיק מידע במאגר הפנימי, אז בדקתי מקור חיצוני של Google Places."
            if language == "he"
            else "I did not find enough information in the internal database, so I checked Google Places as an external source."
        ),
    }
...
- Use ONLY the data in the provided data_payload for factual restaurant details. If data_payload.source is Google Places API, mention that the external information may change.
...
def build_agent_response(message: str, parsed: Dict[str, Any]) -> Dict[str, str]:
    ...
    if intent == "reviews":
        if restaurant is None:
            if parsed.get("restaurant_name") or params.get("city") or params.get("region"):
                payload = external_places_payload(message, parsed, language, params, reason="restaurant_reviews_not_in_internal_database")
                if payload.get("places"):
                    answer = ask_ai_to_answer_from_data(message, "reviews_external", payload, language)
                    remember_conversation(message, answer)
                    return {"message": answer}
        ...
        if restaurant_for_city is None:
            parsed_with_restaurant = {
                **parsed,
                "restaurant_name": clean_name(str(restaurant.get("name_he") or restaurant.get("name"))),
            }
            payload = external_places_payload(message, parsed_with_restaurant, language, params, reason="branch_not_found_for_reviews")
            if payload.get("places"):
                payload["base_restaurant"] = restaurant_row_to_dict(restaurant)
                answer = ask_ai_to_answer_from_data(message, "reviews_external", payload, language)
                remember_conversation(message, answer)
                return {"message": answer}
    ...
    if intent == "rating":
        if restaurant is None:
            if parsed.get("restaurant_name") or params.get("city") or params.get("region"):
                payload = external_places_payload(message, parsed, language, params, reason="rating_not_in_internal_database")
                if payload.get("places"):
                    answer = ask_ai_to_answer_from_data(message, "rating_external", payload, language)
                    remember_conversation(message, answer)
                    return {"message": answer}
    ...
    if intent == "peak_hours":
        ...
        if restaurant_for_city is None:
            ...
            payload = external_places_payload(message, parsed_with_restaurant, language, params, reason="branch_not_found_for_peak_hours")
            if payload.get("places"):
                payload["note"] = "Google Places may provide opening hours and rating, but official Google Places API does not provide popular-times/peak-hour counts."
                answer = ask_ai_to_answer_from_data(message, "peak_hours_external", payload, language)
                remember_conversation(message, answer)
                return {"message": answer}
    ...
    if intent in ["recommendation", "menu", "general_restaurant_question", "unclear"]:
        ...
        # אם גם אחרי fallback אין תוצאות פנימיות — פונים ל-Google Places.
        if recs.empty:
            payload = external_places_payload(message, parsed, language, params, reason="no_internal_recommendations_found")
            if payload.get("places"):
                answer = ask_ai_to_answer_from_data(message, "recommendation_external", payload, language)
                remember_conversation(message, answer)
                return {"message": answer}
    ...
    # שאלות כלליות על מסעדה/שעות פתיחה/כתובת שלא נמצאות במאגר — Google Places fallback.
    if intent in ["general_restaurant_question", "menu", "unclear"] and (parsed.get("restaurant_name") or params.get("city") or params.get("region")):
        payload = external_places_payload(message, parsed, language, params, reason="general_question_not_answered_by_internal_database")
        if payload.get("places"):
            answer = ask_ai_to_answer_from_data(message, "external_general_restaurant_question", payload, language)
            remember_conversation(message, answer)
            return {"message": answer}
...
def answer_free_text(message: str) -> Dict[str, str]:
    ...
    # מתכונים והוראות הכנה הם מחוץ לתחום: SmartBite הוא סוכן מסעדות, לא סוכן בישול.
    if is_recipe_request(message):
        if get_openai_client() is None:
            return {"message": SMARTBITE_DOMAIN_MESSAGE_HE if language == "he" else SMARTBITE_DOMAIN_MESSAGE_EN}
        answer = ask_ai_to_answer_from_data(
            message,
            "off_topic_recipe",
            {
                "type": "off_topic_recipe",
                "allowed_domain": "restaurant recommendations and restaurant information",
                "fallback_answer": (
                    "אני מתמחה במסעדות ולא במתכונים. אם תרצי, אוכל לעזור למצוא מסעדה עם פסטה טובה 😊"
                    if language == "he"
                    else "I specialize in restaurants, not recipes. I can help you find a great place for pasta 😊"
                ),
            },
            language,
        )
