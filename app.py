...
except Exception:  # keeps local/server startup safe if package is missing
    OpenAI = None
...
def plan_with_gpt(message: str) -> Dict[str, Any]:
    ...
    if client is None:
        ...
        return {
            "language": language,
            "intent": hint.get("intent") or "recommendation",
            "is_restaurant_related": hint.get("is_restaurant_related", True),
...
def answer_with_gpt(message: str, plan: Dict[str, Any], tool_results: Dict[str, Any]) -> str:
    ...
    if client is None:
        return tool_results.get("fallback_answer") or (
            "אני יכולה לעזור עם מסעדות, המלצות, ביקורות, מנות ושעות עומס בישראל 🍽️"
            if language == "he" else
            "I can help with restaurants, recommendations, reviews, dishes and peak hours in Israel 🍽️"
        )

    history = _context.get("conversation_history", [])[-8:]
    system_prompt = """
You are SmartBite, a real AI restaurant agent.
...
- If internal data is not available and external/ChatGPT knowledge is used, clearly say that it is not from the internal SmartBite database.
...
"""
    try:
        ...
        print("Final answer error:", exc)

    return tool_results.get("fallback_answer") or (
        "אני כאן לעזור עם מסעדות בישראל 🍽️ אפשר לשאול על המלצות, ביקורות, שעות עומס או מנות מומלצות."
        if language == "he" else
        "I’m here to help with restaurants in Israel 🍽️ Ask me about recommendations, reviews, peak hours, or dishes."
    )
