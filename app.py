...
def is_short_location_reply(message: str) -> bool:
    msg = normalize_text(message)
    params = extract_local_params(message)
    return bool((params.get("city") or params.get("region")) and len(msg.split()) <= 4)


def infer_local_intent(message: str) -> Optional[str]:
    msg = normalize_text(message)
    if any(w in msg for w in ["ביקורת", "ביקורות", "חוות דעת", "reviews", "review"]):
        return "reviews"
    if any(w in msg for w in ["שעות עומס", "עומס", "עמוס", "busy", "peak"]):
        return "peak_hours"
    if any(w in msg for w in ["שעות פתיחה", "פתוח", "פתיחה", "opening", "hours"]):
        return "opening_hours"
    if any(w in msg for w in ["דירוג", "כוכבים", "rating"]):
        return "rating"
    if any(w in msg for w in ["תפריט", "מנות", "מנה", "menu"]):
        return "menu"
    if any(w in msg for w in ["מומלצת", "מומלץ", "recommended dish"]):
...
def chain_candidates_for_restaurant(row: pd.Series) -> pd.DataFrame:
    df = load_restaurants()
    if df.empty or row is None:
        return pd.DataFrame()
    base_name = clean_name(str(row.get("name_he") or row.get("name")))
...
def city_options_for_chain(row: pd.Series, limit: int = 8) -> List[str]:
    candidates = chain_candidates_for_restaurant(row)
    if candidates.empty:
        return []
    vals = []
    for _, r in candidates.iterrows():
        city = str(r.get("city_he") or r.get("city") or "").strip()
        if city and city not in vals:
            vals.append(city)
        if len(vals) >= limit:
            break
    return vals
...
def local_intent_hint(message: str) -> Dict[str, Any]:
    ...
    inferred = infer_local_intent(message)
    if inferred:
        return {"intent": inferred, "is_restaurant_related": True}
    return {}
...
def plan_with_gpt(message: str) -> Dict[str, Any]:
    ...
    # Fast path: for clear local intents/filters, avoid an extra Gemini planner call.
    # This makes follow-up answers like "רמת גן" and requests for reviews/similar restaurants much faster and more reliable.
    local_params = extract_local_params(message)
    msg_norm = normalize_text(message)
    pending = _context.get("pending_question") or {}
    if hint or has_any_param(local_params) or (pending and is_short_location_reply(message)) or any(w in msg_norm for w in ["היי", "שלום", "מה קורה", "מה נשמע", "תודה", "hello", "hi"]):
        return local_plan_fallback(message, language, hint)
...
def local_plan_fallback(message: str, language: str, hint: Dict[str, Any], error: Optional[str] = None) -> Dict[str, Any]:
    ...
    if hint.get("intent") == "off_topic":
        intent = "off_topic"
        related = False
    elif hint.get("intent"):
        intent = hint.get("intent")
        related = hint.get("is_restaurant_related", True)
...
def local_natural_answer(message: str, plan: Dict[str, Any], tool_results: Dict[str, Any]) -> str:
    ...
    if t == "recommendations" and tool_results.get("restaurants"):
        ...
        intro = "מצאתי לך כמה אפשרויות מתאימות לפי הדאטה של SmartBite 😊"
    ...
    if tool_results.get("restaurant"):
        ...
        if tool_results.get("reviews", {}).get("available"):
            ...
            sample_txt = ""
            if samples:
                sample_txt = "\n\nכמה דוגמאות מביקורות במאגר:\n" + "\n".join([f"• {txt}" for txt in samples[:3]])
            return f"לפי הדאטה של SmartBite, ל־{r.get('name')} יש {rev.get('count')} ביקורות, עם דירוג ביקורות ממוצע של {rev.get('average_review_rating', r.get('rating'))}." + sample_txt
        if tool_results.get("peak_hours", {}).get("available"):
            hours = ", ".join([f"{x['hour']}:00" for x in tool_results["peak_hours"].get("peak_hours", [])])
            return f"לפי נתוני ההזמנות של SmartBite, שעות העומס הבולטות של {r.get('name')} הן: {hours}."
    ...
    if tool_results.get("external", {}).get("available"):
        ...
        for i, place in enumerate(places[:3], 1):
            ...
            if place.get("google_maps_url"):
                line.append(f"Google Maps: {place.get('google_maps_url')}")
            cards.append("\n".join(line))
...
def answer_with_gpt(message: str, plan: Dict[str, Any], tool_results: Dict[str, Any]) -> str:
    ...
    # Speed optimization: when we already have concrete DB/Google tool results, format them locally.
    # Gemini is still used for smalltalk/general restaurant knowledge, but data answers do not need a second network call.
    if tool_results.get("type") in ["recommendations", "similar_restaurants", "missing_info", "off_topic"] or tool_results.get("restaurant") or tool_results.get("external", {}).get("available"):
...
def run_internal_tools(message: str, plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if intent in ["reviews", "rating", "peak_hours", "opening_hours", "recommended_dish", "menu", "general_restaurant_question"] and restaurant is not None:
        ...
        # For reviews on a chain with several branches, ask for city/area first instead of choosing a random branch.
        if intent == "reviews" and not (params.get("city") or params.get("region")):
            chain_candidates = chain_candidates_for_restaurant(target)
            if not chain_candidates.empty and len(chain_candidates) > 1:
                options = city_options_for_chain(target, limit=8)
                _context["pending_question"] = {
                    "intent": "reviews",
                    "restaurant_id": safe_int(target.get("restaurant_id")),
                    "params": params,
                }
                result.update({
                    "type": "missing_info",
                    "missing": "city_for_reviews",
                    "internal_found": True,
                    "fallback_answer": f"מצאתי כמה סניפים של {clean_name(str(target.get('name_he') or target.get('name')))}. באיזו עיר או אזור תרצי שאבדוק את הביקורות?" + ("\nאפשרויות לדוגמה: " + ", ".join(options) if options else ""),
                })
                return result
