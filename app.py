...
# שאלות שנועדו למצוא מידע על מסעדה ספציפית, ולא להפעיל מנוע המלצות רגיל.
LOOKUP_WORDS = [
    "ספר לי על", "תספר לי על", "פרטים על", "מידע על", "פירוט על", "ספר בפירוט", "תספר בפירוט",
    "כתובת", "קישור", "מפות", "מפה", "google maps", "גוגל מפס", "גוגל maps",
    "טלפון", "מספר טלפון", "אתר", "איפה נמצאת", "איפה נמצא", "איך מגיעים",
    "tell me about", "address", "maps", "map link", "phone", "website", "details about",
]
...
def detect_intent(message: str) -> str:
    ...
    if any(w in msg for w in OFF_TOPIC_WORDS):
        return "off_topic"
    # Lookup must be detected before generic recommendation/rating logic,
    # otherwise questions like "ספר לי על OCD" may be treated as normal recommendations.
    if any(normalize_text(w) in msg for w in LOOKUP_WORDS):
...
def build_external_lookup_query(message: str, params: Dict[str, Any], candidates: pd.DataFrame) -> str:
    """Build a focused Google Places query for a specific restaurant lookup.
    This function is intentionally narrow so it will not affect the normal recommendation flow.
    """
    city = display_city(params.get("city")) if params.get("city") else ""
    if not candidates.empty:
        base = clean_name(candidates.iloc[0].get("name_he") or candidates.iloc[0].get("name"))
        parts = [base, city, "מסעדה"]
        return " ".join([p for p in parts if p]).strip()

    # Keep the original restaurant name in the query, but remove common command words.
    q = message
    for word in LOOKUP_WORDS:
        q = re.sub(re.escape(word), " ", q, flags=re.I)
    q = re.sub(r"\s+", " ", q).strip()
    if city and city not in q:
        q = f"{q} {city}"
    return f"{q} מסעדה ישראל".strip()
...
def format_google_places_lookup(ext: Dict[str, Any], language: str = "he") -> str:
    if not ext.get("available") or not ext.get("places"):
        return "לא מצאתי פרטים חיצוניים על המסעדה הזו כרגע."
    place = ext["places"][0]
    lines = [short_place_description(place, language)]
    name = place.get("name")
    if name:
        lines.append(f"\n**{name}**")
    if place.get("address"):
        lines.append(f"כתובת: {place.get('address')}")
    if place.get("rating"):
        rating_line = f"דירוג: {place.get('rating')}"
        if place.get("user_rating_count"):
            rating_line += f" ({place.get('user_rating_count')} ביקורות)"
        lines.append(rating_line)
    if place.get("phone"):
        lines.append(f"טלפון: {place.get('phone')}")
    if place.get("google_maps_url"):
        lines.append(f"[פתח ב-Google Maps]({place.get('google_maps_url')})")
    if place.get("website"):
        lines.append(f"[אתר המסעדה]({place.get('website')})")
    hours = place.get("opening_hours") or {}
    weekday = hours.get("weekdayDescriptions") if isinstance(hours, dict) else None
    if weekday:
        lines.append("שעות פתיחה:")
        lines.extend([f"• {x}" for x in weekday[:7]])
    return "\n".join(lines)
