...
INTENT_WORDS = {
    "restaurant_lookup": ["כתובת", "קישור", "מפות", "מפה", "google maps", "גוגל מפס", "טלפון", "מספר טלפון", "ספר לי על", "פרטים על", "איפה נמצאת", "איפה נמצא", "איפה", "address", "maps", "phone", "tell me about"],
...
def extract_restaurant_name_for_lookup(message: str, candidates: pd.DataFrame) -> str:
    """Extracts the restaurant/place name for external lookup queries.
    This is intentionally simple and deterministic so a lookup like
    "ספר לי על מסעדת OCD בתל אביב" does not become a generic recommendation.
    """
    if not candidates.empty:
        return clean_name(candidates.iloc[0].get("name_he") or candidates.iloc[0].get("name"))

    original = (message or "").strip()
    # remove known city/region words so the place name remains cleaner
    cleaned = original
    for city_he in sorted(CITY_MAP.keys(), key=len, reverse=True):
        cleaned = re.sub(re.escape(city_he), " ", cleaned, flags=re.I)
    for region_he in sorted(REGION_MAP.keys(), key=len, reverse=True):
        cleaned = re.sub(re.escape(region_he), " ", cleaned, flags=re.I)

    patterns = [
        r"(?:ספר(?:י)?\s+לי\s+על\s+מסעדת?\s+)(.+)",
        r"(?:ספר(?:י)?\s+לי\s+על\s+)(.+)",
        r"(?:קישור\s+ל(?:מפות|גוגל\s*מפס)?\s+של\s+)(.+)",
        r"(?:כתובת\s+.*?של\s+)(.+)",
        r"(?:טלפון\s+.*?של\s+)(.+)",
        r"(?:איפה\s+(?:נמצאת|נמצא)\s+)(.+)",
    ]
    for pat in patterns:
        m = re.search(pat, cleaned, flags=re.I)
        if m:
            name = m.group(1).strip()
            name = re.sub(r"\b(?:מסעדה|מסעדת|בישראל|ישראל|restaurant)\b", " ", name, flags=re.I)
            name = re.sub(r"[?!.:,;]+", " ", name)
            name = re.sub(r"\s+", " ", name).strip()
            if name:
                return name

    # fallback: remove lookup words and keep remaining meaningful text
    name = normalize_text(cleaned)
    for w in ["תן לי", "תני לי", "את", "הכתובת", "כתובת", "קישור", "למפות", "מפות", "מפה", "גוגל מפס", "של", "ספר לי על", "מסעדת", "מסעדה", "טלפון", "פרטים על"]:
        name = name.replace(normalize_text(w), " ")
    name = re.sub(r"\s+", " ", name).strip()
    return name or original


def build_lookup_query(message: str, candidates: pd.DataFrame, params: Dict[str, Any]) -> str:
    name = extract_restaurant_name_for_lookup(message, candidates)
    city = display_city(params.get("city")) if params.get("city") else ""
    region = display_region(params.get("region")) if params.get("region") and not city else ""
    location = city or region
    return " ".join([x for x in [name, location, "restaurant Israel"] if x]).strip()


def format_google_place_lookup(message: str, params: Dict[str, Any], candidates: pd.DataFrame) -> Optional[str]:
    query = build_lookup_query(message, candidates, params)
    external = search_google_places(query, language=detect_language(message))
    if not external.get("available"):
        return None
    places = external.get("places") or []
    if not places:
        return None

    lines = []
    # For a direct lookup, usually the first result is enough; include up to 2 only if the query is ambiguous.
    for i, place in enumerate(places[:2], 1):
        name = place.get("name") or "מסעדה"
        address = place.get("address")
        rating = place.get("rating")
        count = place.get("user_rating_count")
        phone = place.get("phone")
        maps = place.get("google_maps_url")
        website = place.get("website")
        prefix = f"{i}. " if len(places[:2]) > 1 else ""
        ...
        if rating:
            ...
            if count:
                rating_line += f" ({count} ביקורות)"
            lines.append(rating_line)
        if phone:
            lines.append(f"טלפון: {phone}")
        if maps:
            lines.append(f"קישור למפות: {maps}")
        if website:
            lines.append(f"אתר: {website}")
        lines.append("")
    return "\n".join(lines).strip()
...
def answer_free_text(message: str) -> Dict[str, str]:
    ...
    if intent == "smalltalk":
        msg=normalize_text(message)
        if any(w in msg for w in ["היי","שלום","hi","hello"]):
            answer="היי, איך אפשר לעזור? אפשר למצוא מסעדות, לבדוק ביקורות או לקבל מידע על מקום מסוים."
        elif "תודה" in msg or "thanks" in msg:
            answer="בשמחה. יש עוד משהו שתרצי לבדוק לגבי מסעדות?"
        else:
            answer = natural_gemini_fallback(message)
        remember_conversation(message, answer)
        return {"message": answer}

    # Direct lookup: address / maps link / phone / details about a specific restaurant.
    # This must not fall into the generic recommendation flow.
    if intent == "restaurant_lookup":
        chain_candidates = find_chain_candidates(message, None)
        answer = format_google_place_lookup(message, params, chain_candidates)
        if answer is None and not chain_candidates.empty:
            # Graceful fallback when Google Places is not available: basic known facts only.
            branch = select_branch(chain_candidates, params) or chain_candidates.sort_values("rating", ascending=False).iloc[0]
            answer = restaurant_card(branch)
        if answer is None:
            answer = "לא הצלחתי למצוא פרטים מדויקים. אפשר לנסות לכתוב את שם המסעדה והעיר בצורה מלאה?"
        remember_conversation(message, answer)
        return {"message": answer}
