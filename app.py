...
def should_ask_city_for_chain(intent: str, candidates: pd.DataFrame, params: Dict[str, Any]) -> bool:
    if candidates.empty or has_location(params):
        return False
    if intent in ["reviews", "rating", "peak_hours", "opening_hours", "recommended_dish", "menu", "similar_restaurants"]:
...
def ask_city(chain_name: str, candidates: pd.DataFrame, intent: str, params: Dict[str, Any]) -> str:
    ...
    _context["pending_question"] = {"intent": intent, "chain_key": message_chain_alias(chain_name) or normalize_text(chain_name).replace(" ", ""), "params": params}
    examples = " / ".join(cities[:5]) if cities else "למשל תל אביב, רמת גן או ירושלים"
    return f"באיזו עיר או אזור תרצי שאבדוק את {clean_name(chain_name)}? יש כמה סניפים. למשל: {examples}."
...
def apply_pending(message: str, intent: str, params: Dict[str, Any]) -> Tuple[str, Dict[str, Any], Optional[str]]:
    ...
    if pending and has_location(params):
        ...
        return intent, merged, pending.get("chain_key")
...
def format_similar(base: pd.Series, params: Dict[str, Any]) -> str:
    ...
    return f"מצאתי מסעדות דומות ל{base_name}:\n\n" + "\n\n".join(cards)
...
def answer_free_text(message: str) -> Dict[str, str]:
    ...
    if intent == "recommendation" and has_any_param(params) and not has_location(params):
        ...
        answer = "באיזו עיר או אזור תרצי שאחפש?"
        remember_conversation(message, answer)
        return {"message": answer}
    ...
    if intent in ["reviews", "rating", "peak_hours", "opening_hours", "recommended_dish", "menu", "similar_restaurants"] and not chain_candidates.empty:
        ...
        if branch is None:
            query = f"{clean_name(chain_name)} {params.get('city') or params.get('region') or ''} restaurant Israel"
            ext = search_google_places(query, language=language)
            if ext.get("available"):
                p = ext["places"][0]
                answer = f"מצאתי את {p.get('name')}. דירוג: {p.get('rating') or 'לא זמין'}. כתובת: {p.get('address') or 'לא זמינה'}."
            else:
                answer = f"לא מצאתי סניף מתאים של {chain_name} בעיר שביקשת."
            remember_conversation(message, answer)
            return {"message": answer}
