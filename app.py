...
def ask_openai_to_format_answer(user_message: str, data_answer: str, language: str) -> str:
    ...
    system_prompt = """
You are SmartBite, a helpful restaurant assistant.
Rewrite the provided database answer into a natural conversational answer.
...
def answer_free_text(message: str) -> Dict[str, str]:
    ...
    if parsed.get("is_restaurant_related") is False or intent == "off_topic":
        answer = SMARTBITE_DOMAIN_MESSAGE_HE if language == "he" else SMARTBITE_DOMAIN_MESSAGE_EN
        remember_conversation(message, answer)
        return {"message": answer}

    if intent == "greeting":
        answer = "היי 😊 איך אפשר לעזור לך בעולם המסעדות?" if language == "he" else "Hi 😊 How can I help you with restaurants?"
        remember_conversation(message, answer)
        return {"message": answer}

    if intent == "thanks":
        answer = "בשמחה 😊" if language == "he" else "You’re welcome 😊"
        remember_conversation(message, answer)
        return {"message": answer}

    if intent == "profile_update":
        answer = "מעולה 😊 איך אוכל לעזור לך?" if language == "he" else "Great 😊 How can I help you?"
        remember_conversation(message, answer)
        return {"message": answer}
    ...
    try:
        ...
        if intent == "recommendation":
            ...
            if not params.get("city") and not params.get("region"):
                ...
                answer = "בשמחה 😊 באיזו עיר או אזור תרצי לחפש?" if language == "he" else "Sure 😊 Which city or region should I search in?"
                remember_conversation(message, answer)
                return {"message": answer}
        ...
        if intent == "reviews":
            ...
            if not params.get("city") and not params.get("region"):
                ...
                answer = "באיזו עיר או אזור תרצי לבדוק את הביקורות?" if language == "he" else "Which city or region should I check the reviews for?"
                remember_conversation(message, answer)
                return {"message": answer}
        ...
        if intent == "peak_hours":
            ...
            if not params.get("city") and not params.get("region"):
                ...
                answer = "באיזו עיר או אזור תרצי לבדוק את שעות העומס?" if language == "he" else "Which city or region should I check peak hours for?"
                remember_conversation(message, answer)
                return {"message": answer}
        ...
        answer = (
            "אפשר לנסח לי קצת יותר מדויק? למשל: המלצה למסעדה, ביקורות, דירוג, שעות עומס או מנה מומלצת."
            if language == "he"
            else "Can you be a bit more specific? For example: recommendation, reviews, rating, peak hours, or recommended dish."
        )
        remember_conversation(message, answer)
        return {"message": answer}
