...
def search_google_places_text(query: str, language: str = "he", limit: int = 5) -> list[Dict[str, Any]]:
    """
    External fallback using Google Places API (New) Text Search.
    ...
    try:
        ...
        print("Google Places search error:", exc)
        return []
...
def build_agent_response(message: str, parsed: Dict[str, Any]) -> Dict[str, str]:
    ...
    if intent == "reviews":
        ...
        if restaurant_for_city is None:
            payload = external_places_payload(message, parsed, language, params, reason="branch_not_found_for_reviews")
            if payload.get("places"):
    ...
    if intent == "peak_hours":
        ...
        restaurant_for_city = find_restaurant_in_location(restaurant, params)
        if restaurant_for_city is None:
            payload = external_places_payload(message, parsed, language, params, reason="branch_not_found_for_peak_hours")
            if payload.get("places"):
