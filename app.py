...
def message_contains_restaurant_alias(message: str) -> bool:
    msg_norm = normalize_text(message)
    msg_compact = msg_norm.replace(" ", "")
    for canonical, aliases in RESTAURANT_ALIASES.items():
        for alias in aliases + [canonical]:
            alias_norm = normalize_text(alias)
            if alias_norm and (alias_norm in msg_norm or alias_norm.replace(" ", "") in msg_compact):
                return True
    return False


def find_restaurant(message: str, explicit_name: Optional[str] = None, use_context: bool = False) -> Optional[pd.Series]:
    """Find a restaurant mentioned in the current message.
    Important: do NOT mix the previous restaurant into the search unless use_context=True.
    Mixing context caused questions about ג'פניקה to accidentally return ארומה.
    """
    df = load_restaurants()
    if df.empty:
        return None

    parts = [message or "", explicit_name or ""]
    if use_context and not explicit_name and not message_contains_restaurant_alias(message):
        parts.append(str(_context.get("last_restaurant_name") or ""))
    search_text = " ".join(parts).strip()
    msg_norm = normalize_text(search_text)
    msg_compact = msg_norm.replace(" ", "")

    # 1) Known aliases. Handles prefixes like במחניודה / לג'פניקה because alias is contained in normalized text.
    for canonical, aliases in RESTAURANT_ALIASES.items():
    ...
    # 3) Any restaurant name in current message only
    for _, row in df.iterrows():
...
Use the conversation history and pending_question. If the user replies with only a city/region/short answer, infer the missing intent from pending_question. For recommendations or restaurant branch-specific questions without a city/region, set missing_info to city_or_region.
...
"""
    try:
        ...
        parsed = local_plan_fallback(message, language, hint, str(exc))
...
def local_plan_fallback(message: str, language: str, hint: Dict[str, Any], error: Optional[str] = None) -> Dict[str, Any]:
    """Create a useful local plan when Gemini planner is unavailable.
    This prevents normal user messages like a city reply from becoming a technical error.
    """
    params = extract_local_params(message)
    msg = normalize_text(message)
    ...
    if hint.get("intent") == "off_topic":
        intent = "off_topic"
        related = False
    elif hint.get("intent") == "similar_restaurants":
        intent = "similar_restaurants"
        related = True
    elif pending and (params.get("city") or params.get("region")):
        intent = pending.get("intent") or "recommendation"
        related = True
    elif any(w in msg for w in ["היי", "שלום", "מה קורה", "מה נשמע", "תודה", "ביי", "hello", "hi", "thanks"]):
        intent = "smalltalk"
        related = True
    elif has_any_param(params):
...
def format_restaurant_card(r: Dict[str, Any], idx: int = 1) -> str:
    parts = [f"**{idx}. {r.get('name', 'מסעדה')}**"]
    city = r.get("city") or r.get("city_en")
    if city:
    ...
    if r.get("similarity_score"):
        parts.append(f"ציון דמיון: {r.get('similarity_score')}%")
    return "\n".join(parts)


def local_natural_answer(message: str, plan: Dict[str, Any], tool_results: Dict[str, Any]) -> str:
    """Natural backup answer for non-technical cases when Gemini final wording fails.
    The technical fallback is kept only for real technical failures with no usable data.
    """
    language = plan.get("language") or detect_language(message)
    intent = tool_results.get("intent") or plan.get("intent")
    t = tool_results.get("type")

    if t == "off_topic" or plan.get("is_restaurant_related") is False:
        return "אני מתמחה בעולם המסעדות והאוכל 🍽️ אשמח לעזור בהמלצות, ביקורות, שעות עומס, מנות, כשרות או מסעדות דומות."

    if t in ["smalltalk", "profile_update"] or intent == "smalltalk":
        return "היי 😊 אני כאן. מה בא לך למצוא היום — מסעדה לפי עיר, סגנון אוכל, תקציב, כשרות או אולי מקום לדייט?"

    if t == "missing_info":
        return tool_results.get("fallback_answer") or "בשמחה 😊 באיזו עיר או אזור תרצי שאחפש?"

    if t == "recommendations" and tool_results.get("restaurants"):
        cards = [format_restaurant_card(r, i + 1) for i, r in enumerate(tool_results["restaurants"][:5])]
        intro = "מצאתי לך כמה אפשרויות מתאימות 😊"
        if tool_results.get("used_relaxed_search"):
            intro += "\nלא הייתה התאמה מלאה לכל הסינונים, אז הרחבתי קצת את החיפוש."
        return intro + "\n\n" + "\n\n".join(cards)

    if t == "similar_restaurants":
        if tool_results.get("restaurants"):
            base = (tool_results.get("base_restaurant") or {}).get("name", "המסעדה שביקשת")
            cards = [format_restaurant_card(r, i + 1) for i, r in enumerate(tool_results["restaurants"][:5])]
            return f"מצאתי מסעדות דומות ל־{base} בעזרת מודל Cosine Similarity, שמשווה מאפיינים כמו מטבח, סוג מסעדה, אזור, מחיר וכשרות:\n\n" + "\n\n".join(cards)
        return tool_results.get("fallback_answer") or "לא הצלחתי לזהות לאיזו מסעדה לחפש דומות."

    if tool_results.get("restaurant"):
        r = tool_results["restaurant"]
        if tool_results.get("reviews", {}).get("available"):
            rev = tool_results["reviews"]
            samples = rev.get("sample_reviews") or []
            sample_txt = ""
            if samples:
                sample_txt = "\n\nכמה דוגמאות מהביקורות:\n" + "\n".join([f"• {x}" for x in samples[:3]])
            return f"ל־{r.get('name')} יש {rev.get('count')} ביקורות, עם דירוג ביקורות ממוצע של {rev.get('average_review_rating', r.get('rating'))}." + sample_txt
        if tool_results.get("peak_hours", {}).get("available"):
            hours = ", ".join([f"{x['hour']}:00" for x in tool_results["peak_hours"].get("peak_hours", [])])
            return f"שעות העומס הבולטות של {r.get('name')} הן: {hours}."
        if tool_results.get("menu", {}).get("available"):
            items = tool_results["menu"].get("items", [])[:5]
            txt = "\n".join([f"• {x.get('item_name')} — ₪{int(x.get('price', 0))}" for x in items])
            return f"מצאתי כמה מנות מהתפריט של {r.get('name')}:\n{txt}"
        return "מצאתי את המסעדה בדאטה של SmartBite:\n\n" + format_restaurant_card(r, 1)

    if tool_results.get("external", {}).get("available"):
        places = tool_results["external"].get("places", [])
        cards = []
        for i, place in enumerate(places[:3], 1):
            line = [f"**{i}. {place.get('name', 'מסעדה')}**"]
            if place.get("address"):
                line.append(f"כתובת: {place.get('address')}")
            if place.get("rating"):
                line.append(f"דירוג: {place.get('rating')}")
            cards.append("\n".join(line))
        return "לא מצאתי תשובה מלאה בדאטה הפנימי, אז בדקתי מקור חיצוני ומצאתי:\n\n" + "\n\n".join(cards)

    if tool_results.get("fallback_answer") and tool_results.get("type") != "technical_error":
        return tool_results["fallback_answer"]
...
def answer_with_gpt(message: str, plan: Dict[str, Any], tool_results: Dict[str, Any]) -> str:
    ...
    if plan.get("intent") == "technical_error" and tool_results.get("type") == "technical_error":
        return technical_error_message(language)
...
- Do not mention whether the information came from the internal database, Google Places, or Gemini. Just answer naturally.
...
- If similar restaurants are requested, present the matches naturally. Do not mention technical implementation unless the user asks.
"""
    try:
    ...
    return local_natural_answer(message, plan, tool_results)
...
def is_branch_specific_needed(intent: str, restaurant: Optional[pd.Series]) -> bool:
    return restaurant is not None and intent in ["reviews", "rating", "peak_hours", "opening_hours", "recommended_dish", "menu", "general_restaurant_question"]


def ask_for_location_for_restaurant(intent: str, restaurant: pd.Series, params: Dict[str, Any]) -> Dict[str, Any]:
    name = clean_name(str(restaurant.get("name_he") or restaurant.get("name")))
    _context["pending_question"] = {
        "intent": intent,
        "params": params,
        "restaurant_id": safe_int(restaurant.get("restaurant_id")),
    ...
    return {
        ...
        "intent": intent,
        "params": params,
        "restaurant": row_to_dict(restaurant),
        "internal_found": True,
        "external_used": False,
        "gemini_general_used": False,
        "fallback_answer": f"באיזו עיר או אזור תרצי שאבדוק את {name}?",
    }


def ask_for_location_for_recommendation(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "external_used": False,
        "gemini_general_used": False,
        "fallback_answer": "בשמחה 😊 באיזו עיר או אזור תרצי שאחפש?",
    }
...
def run_internal_tools(message: str, plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    restaurant = find_restaurant(message, plan.get("restaurant_name"), use_context=False)
    ...
    # For recommendations without city/region, ask for location first instead of returning random branches or external places.
    if intent in ["recommendation", "unclear"] and has_any_param(params) and not has_location(params):
        return ask_for_location_for_recommendation(params)

    # For a chain/specific restaurant question without city/region, ask which branch/location first.
    if is_branch_specific_needed(intent, restaurant) and not has_location(params):
        return ask_for_location_for_restaurant(intent, restaurant, params)
    ...
    result["gemini_general_used"] = True
    result["type"] = "gemini_restaurant_knowledge_fallback"
    result["fallback_answer"] = "לא מצאתי תשובה מדויקת כרגע. אפשר לנסח שוב או להוסיף עיר/אזור?"
    return result
...
def api_chat():
    try:
        ...
        return jsonify({"message": technical_error_message("he")})
