...
def technical_error_message(language: str = "he") -> str:
    if language == "en":
        return "Sorry, I’m having a technical connection issue right now. Please try again in a moment."
    return "מצטערת, יש כרגע תקלה טכנית בחיבור למערכת. נסי שוב בעוד רגע."


def sanitize_payload_for_gpt(value: Any) -> Any:
    """
    לא מעבירים ל-GPT תשובות fallback קבועות כמו שהן, כדי שלא יחזור עליהן בצורה רובוטית.
    במקום זאת הן הופכות לרמז פנימי בלבד.
    """
    if isinstance(value, dict):
        clean = {}
        for k, v in value.items():
            if k == "fallback_answer":
                clean["internal_hint"] = v
            else:
                clean[k] = sanitize_payload_for_gpt(v)
        return clean
    if isinstance(value, list):
        return [sanitize_payload_for_gpt(v) for v in value]
    return value

def ai_final_answer(user_message: str, parsed: Dict[str, Any], data_payload: Dict[str, Any]) -> str:
    """
    זה המקום היחיד שבו נוצרת תשובה למשתמש.
    כלומר: כל השיח מנוהל ע"י ChatGPT.
    fallback קבוע מופעל רק במקרה של תקלה טכנית אמיתית: אין API key / שגיאת OpenAI.
    """
    client = get_openai_client()
    ...
    system_prompt = """
You are SmartBite, a real ChatGPT-style AI Agent for restaurants in Israel.

Conversation behavior:
- You, ChatGPT, manage the conversation naturally from start to finish.
- Do NOT sound like a scripted chatbot.
- Do NOT copy internal hints or fallback text verbatim.
- For greetings or casual messages, respond naturally and warmly, then gently steer toward restaurants/food if helpful.
- Do not list all capabilities every time. Mention only what is relevant.
...
Knowledge and tools:
- The internal SmartBite database is the first and preferred factual source.
- If internal_data is provided, use it as the primary source.
- If external Google Places data is provided, use it as the second source.
- If neither internal nor external data is enough, you may answer from general restaurant-domain knowledge, but do not invent live facts such as exact opening hours, current ratings, exact addresses, or prices.

Domain boundaries:
- SmartBite is only for restaurants, food outside the home, recommendations, reviews, ratings, peak hours, opening hours, locations, menus, dishes, kosher, prices, cities, cuisines, and similar restaurants.
- Do not answer unrelated topics such as cars, coding, medicine, weather, politics, flights, hotels, or homework.
- Recipes are outside scope. If asked for a recipe, say you focus on restaurants and can help find a place that serves that dish.

Language:
- Answer in Hebrew when language is "he".
- Answer in English when language is "en".
- Keep the tone friendly, conversational, concise, and helpful.
"""

    clean_payload = sanitize_payload_for_gpt(data_payload)
    ...
    try:
                    ...
                    "content": json.dumps(
                        {
                            ...
                            "instruction": (
                                "Write the final answer to the user naturally. "
                                "Use internal data first, external data second, and general restaurant knowledge only if data is missing. "
                                "Only use a domain limitation response if the user is actually off-topic."
                            ),
                        },
                        ensure_ascii=False,
                    ),
        ...
        print("OpenAI final answer technical error:", exc)
...
def build_internal_response(message: str, parsed: Dict[str, Any]) -> Tuple[Dict[str, Any], bool]:
    ...
    return {
        "type": "unclear",
        "conversation_mode": "natural_chat",
        "note": "The user message is unclear. Ask a short natural clarification related to restaurants only if needed."
    }, True
...
def answer_free_text(message: str) -> Dict[str, str]:
    ...
    if not message:
        # גם כאן אין צורך במשפט רובוטי ארוך — רק בקשת המשך קצרה.
        return {"message": "אני כאן 😊 מה תרצי לשאול על מסעדות?" if language == "he" else "I’m here 😊 What would you like to ask about restaurants?"}

    # אם אין חיבור ל-OpenAI זו תקלה טכנית מבחינת הארכיטקטורה החדשה.
    # רק במקרה כזה משתמשים ב-fallback טכני.
    if get_openai_client() is None:
        return {"message": technical_error_message(language)}
    ...
    try:
        ...
        # לא נמצא מספיק בדאטה הפנימי -> עוברים ל-Google Places -> ואז GPT מנסח תשובה.
        params = merge_params(
        ...
        if internal_payload.get("base_restaurant"):
            restaurant = find_restaurant_by_name(
                internal_payload["base_restaurant"].get("clean_name")
                or internal_payload["base_restaurant"].get("name")
            )
        if restaurant is None:
            restaurant = find_restaurant_by_name(parsed.get("restaurant_name") or message) or get_last_restaurant()
        ...
        answer = ai_final_answer(message, parsed, {
            ...
            "note": (
                "Internal database was not sufficient. Use Google Places if it contains results. "
                "If it does not, answer from general restaurant-domain knowledge without inventing live facts, or ask a clarification."
            ),
        })
        remember_conversation(message, answer)
        return {"message": answer}
        ...
        print("SmartBite technical error:", exc)
        return {"message": technical_error_message(language)}
